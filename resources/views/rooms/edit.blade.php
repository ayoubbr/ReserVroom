        ...
        <div class="relative py-3 sm:max-w-xl sm:mx-auto">
            <div class="relative px-4 py-10 bg-white mx-8 md:mx-0 shadow rounded-3xl sm:p-10">
                <div class="max-w-md mx-auto">
                    ...
                    <form action="{{ route('rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data" class="divide-y divide-gray-200">
                        @csrf
                        @method('PUT')
                        <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                            <div class="flex flex-col">
                                <label class="leading-loose">Room Title</label>
                                <input type="text" name="title" required value='{{$room->title}}'
                            ...
                            <div class="flex flex-col">
                                ...
                                <textarea name="description" required value='{{$room->description}}'
                                    class="px-4 py-2 border focus:ring-blue-500 focus:border-blue-500 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none"
                                    rows="4">{{$room->description}}</textarea>
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">Number of Seats</label>
                                <input type="number" name="seats" required min="1" value='{{$room->seats}}'
                                    class="px-4 py-2 border focus:ring-blue-500 focus:border-blue-500 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none">
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">City</label>
                                <input type="text" name="city" required value='{{$room->city}}'
                                    class="px-4 py-2 border focus:ring-blue-500 focus:border-blue-500 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none">
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">Price per Night</label>
                                <input type="number" name="price" required min="0" step="0.01" value='{{$room->price}}'
                                    class="px-4 py-2 border focus:ring-blue-500 focus:border-blue-500 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none">
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">Room Photo</label>
                                <input type="text" name="photo" required value='{{$room->photo}}'
                                class="px-4 py-2 border focus:ring-blue-500 focus:border-blue-500 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none">
                            </div>
                        </div>
